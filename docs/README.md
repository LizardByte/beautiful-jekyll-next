...
Some optional features load browser scripts from third-party CDNs. When those scripts are versioned and immutable,
prefer pinned CDN URLs with a matching Subresource Integrity hash. The central Renovate preset uses Renovate's HTML
manager for cdnjs `<script>` and `<link>` tags, and custom regex managers for Jekyll config/front matter entries.
Only add SRI where Renovate can maintain it or where a manual review is acceptable. For CDN providers that do not
expose those hashes, use a pinned provider URL and document the Sonar suppression on the flagged line. Prefer cdnjs
when it serves a compatible package version; otherwise use a versioned npm CDN URL. Site-wide `site-css` and `site-js`
entries support SRI the same way as `ext-css` and `ext-js`: use either existing string values or objects with `href`
and optional `sri`. SRI values may use `sha256`, `sha384`, or `sha512`.
...
| ext-js         | List of external JavaScript files to include in the page (eg. `//cdnjs.cloudflare.com/ajax/libs/underscore.js/1.8.2/underscore-min.js`). External JavaScript files that support [Subresource Integrity (SRI)](https://developer.mozilla.org/en-US/docs/Web/Security/Subresource_Integrity) can be specified using the `href` and `sri` parameters. SRI values may use `sha256`, `sha384`, or `sha512` eg.<br/>`href: "//code.jquery.com/jquery-3.1.1.min.js"`<br/>`sri: "sha384-3ceskX3iaEnIogmQchP8opvBy3Mi7Ce34nWjpBIwVTHfGYWQS9jwHDVRnpKKHJg7"` |
